__artifacts_v2__ = {
    'Ph10_1AssetshaveembeddedfilesPhDaPsql': {
        'name': 'Ph10.1-Assets have embedded files-PhDaPsql',
        'description': 'Parses basic asset row data from Photos.sqlite for assets that have embedded files'
                       ' records for a variety of data. This parser should be used in conjunction with other'
                       ' parsers to review a complete row for analysis. The results of this parser could'
    ...
    'Ph10_2AssetshaveembeddedfilesSyndPL': {
        'name': 'Ph10.2-Assets have embedded files-SyndPL',
        'description': 'Parses basic asset row data from Syndication.photoslibrary-Photos.sqlite'
                       ' for assets that have embedded files records for a variety of data. This parser'
                       ' should be used in conjunction with other parsers to review a complete row for analysis.'
...
@artifact_processor
def Ph10_1AssetshaveembeddedfilesPhDaPsql(files_found, report_folder, seeker, wrap_text, timezone_offset):
    ...
    if version.parse(iosversion) <= version.parse("10.3.4"):
        logfunc("Unsupported ios version for PhotosData-Photos.sqlite iOS " + iosversion)
        return (), [], source_path
    if (version.parse(iosversion) >= version.parse("11")) & (version.parse(iosversion) < version.parse("13")):
        ...
        data_headers = ('zAsset-Date Created-0',
                        ...
                        'zAddAssetAttr-zPK-46',
                        'zAsset-UUID = store.cloudphotodb-47',
                        'zAddAssetAttr-Original Stable Hash-48',
                        'zAddAssetAttr.Adjusted Stable Hash-49')
        data_list = get_sqlite_db_records(source_path, query)

        return data_headers, data_list, source_path

@artifact_processor
def Ph10_2AssetshaveembeddedfilesSyndPL(files_found, report_folder, seeker, wrap_text, timezone_offset):
    for source_path in files_found:
        source_path = str(source_path)
    ...
    iosversion = scripts.artifacts.artGlobals.versionf
    if version.parse(iosversion) <= version.parse("10.3.4"):
        logfunc("Unsupported ios version for Syndication.photoslibrary iOS " + iosversion)
        return (), [], source_path
    if (version.parse(iosversion) >= version.parse("11")) & (version.parse(iosversion) < version.parse("13")):
        source_path = get_file_path(files_found,"Photos.sqlite")
        if source_path is None or not os.path.exists(source_path):
